    ...
    <div v-else>
          ...
          <table class="reports-table">
            ...
            <tbody>
              <tr v-for="q in quarterlyData" :key="q.quarter">
                ...
                <td>{{ formatInteger(q.total_orders) }}</td>
          ...
          <table class="reports-table">
            ...
            <tbody>
              <tr v-for="(month, index) in monthlyData" :key="month.month">
                ...
                <td>{{ formatInteger(month.order_count) }}</td>
      ...
      <div class="stats-grid">
        ...
        <div class="stat-card">
          ...
          <div class="stat-value">{{ formatInteger(totalOrders) }}</div>
        </div>
        <div class="stat-card">
...
export default {
  ...
  setup() {
    ...
    // Shared filters — Reports' backend endpoints don't accept filter params today
    // (see server/main.py /api/reports/*), so we refetch on change as a UX hook for
    // future server-side filtering. The watcher avoids the "filters do nothing" feel.
    const {
      selectedPeriod,
      selectedLocation,
      selectedCategory,
    ...
    const totalOrders = computed(() => {
      return monthlyData.value.reduce((sum, m) => sum + (m.order_count || 0), 0)
    })

    const bestQuarter = computed(() => {
      if (quarterlyData.value.length === 0) return ''
      const best = quarterlyData.value.reduce((acc, q) => {
        return (q.total_revenue || 0) > (acc.total_revenue || 0) ? q : acc
    ...
    const loadData = async () => {
      try {
        ...
        error.value = t('reports.errors.failedToLoad') + ': ' + err.message
    ...
    // Reload on global filter changes (backend reports endpoints currently ignore
    // filter params; this still keeps the UI feeling reactive to FilterBar moves).
    ...
    const formatCurrency = (value) => {
      ...
      return currencySymbol.value + num.toLocaleString(localeTag.value, {
    ...
    const formatInteger = (value) => {
      const num = Number(value) || 0
      return num.toLocaleString(localeTag.value)
    }
    ...
    const formatMonth = (monthStr) => {
      ...
      if (isNaN(year) || isNaN(monthIndex)) return monthStr
